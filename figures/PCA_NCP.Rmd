...
```{r, echo= FALSE, fig.width= 25, fig.height=15, warning=FALSE, message=FALSE, results='hide'}
...
# ggsave(filename = here("figures","NCP_correlation_with_biomass.png"), plot, width = 22, height =14 )
...
```{r echo=FALSE, fig.height=15, fig.width=25, message=FALSE, warning=FALSE, results='hide'}
...
# ggsave(filename = here("figures","NCP_correlation_with_biodiversity.png"), plot, width = 22, height =14 )
plot
```
...
corrplot(M, order = 'hclust', hclust.method = 'ward.D2', addrect = 10,  tl.srt = 60)
```
...
grp_3cat <- c(recycling_N="regulating", recycling_P="regulating",Productivity="material",taxo_richness="non material", funct_entropy="regulating", funct_distinctiveness="non material", Selenium_C="material", Zinc_C="material", Omega_3_C="material", Calcium_C="material",Iron_C="material", Vitamin_A_C="material", phylo_entropy="regulating", ED_Mean="non material", aesthe_survey="non material", iucn_species="non material", elasmobranch_diversity="non material", low_mg_calcite="regulating", high_mg_calcite="regulating", aragonite="regulating", monohydrocalcite="regulating", amorphous_carbonate="regulating", biom_lowTL="regulating", biom_mediumTL="regulating", biom_highTL="regulating", fishery_biomass="material") 

grp_3cat <- as.factor(grp_3cat)


fviz_pca_var(pca, col.var = grp_3cat,
             palette = c( "dodgerblue3","darkred", "forestgreen"),
             legend.title = "Nature Based Contributions")
```
...
hist <- factoextra::fviz_eig(pca_regulating, addlabels = TRUE, ylim = c(0, 45), barfill = "forestgreen", barcolor = "forestgreen")

# var <- get_pca_var(pca_regulating)
# corrplot(var$contrib, is.corr=FALSE)    

pca_plot <- fviz_pca_var(pca_regulating, col.var = "forestgreen",
             repel = TRUE 
)
...
#### non_material NCP
```{r, echo= FALSE, warning=FALSE, fig.width= 12, fig.height= 7}
non_material <- names(grp_3cat)[ grp_3cat == "non material" ]
NCP_non_material <- NCP_site[,non_material]
rownames(NCP_non_material) <- rownames(NCP_site)

pca_non_material <- FactoMineR::PCA(NCP_non_material, scale.unit = T, graph=F, ncp=10)

hist <- factoextra::fviz_eig(pca_non_material, addlabels = TRUE, ylim = c(0, 35), barfill = "darkred", barcolor = "darkred")

# var <- get_pca_var(pca_non_material)
# corrplot(var$contrib, is.corr=FALSE)

pca_plot <- fviz_pca_var(pca_non_material, col.var = "darkred",
             repel = TRUE
)
...
fviz_pca_var(pca, col.var = grp_3cat,
             axes=c(3,4),
             palette = c( "darkblue","darkred", "forestgreen"),
             legend.title = "Nature Based Contributions")
```
...
```
  ...
  **/!\ les axes sont changeés: axes 1 et 3**
...
#filter biomass data
site_family_pbiom_rda <- dplyr::filter(site_family_pbiom,
                                       SiteCode %in% NCP_site$SiteCode) %>%
  dplyr::select( - SiteCode)

summary(site_family_pbiom_rda)


#reduce data
NCP_site_for_pca <- NCP_site_for_pca[c(1:1000), c(1:12)]
site_family_pbiom_rda <- site_family_pbiom_rda[c(1:1000),]
#compute RDA
rdacca.hp::rdacca.hp(dv = NCP_site_for_pca, iv = site_family_pbiom_rda,
