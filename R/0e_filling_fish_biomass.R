...
sp_biomna<-taxa_biomna[! str_detect(taxa_biomna,"spp")]
...
lw_sp_biomna<-list()
for (k in sp_biomna) {
  lw_sp_biomna[[k]]<-tryCatch( fishflux::find_lw(sub("_"," ",k), mirror = "de"), 
...
# checking
lw_sp_biomna_df %>% filter(lwa_m=="NA") 
# => NA for 1 species Alectis indicus
...
biom_na<-rls_trop_fish_sizeok %>%
  filter( is.na(biomass) ) 
nrow(biom_na) # 9 cases
n_distinct(biom_na$SurveyID) # 8 surveys
...
rls_trop_fish_sizeok_biomok <- rls_trop_fish_sizeok %>%
  filter ( !(SurveyID %in% unique(biom_na$SurveyID) ) )


# diversity remaining
n_distinct(rls_trop_fish_sizeok_biomok$SurveyID) # 3 631 surveys
n_distinct(rls_trop_fish_sizeok_biomok$family) # 81 families
n_distinct(rls_trop_fish_sizeok_biomok$taxa) # 1 678 taxa
