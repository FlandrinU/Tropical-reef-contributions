...
p_val <- cor.test.p(as.data.frame(dplyr::select(NCP_site_clean, -Biomass)))
summary(p_val)
which(p_val > 0.01)

# Study correlogramm
M <- cor(dplyr::select(NCP_site_clean, -Biomass))
summary(M[which(p_val > 0.01)])
# Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# -0.071440 -0.023364  0.017516  0.009506  0.048354  0.072241 
...
hist(pairwise_corr)
length(which(pairwise_corr > 0.2)) #150
length(which(pairwise_corr < -0.2)) #54
length(which( pairwise_corr < 0.2 & -0.2 < pairwise_corr)) #202
length(which( pairwise_corr < 0.5 & -0.5 < pairwise_corr)) #353
...
plot_mpa <-function(NCP_site, xlim=c(-180,180), ylim = c(-36, 31), legend_pos = "none"){
  ggplot(NCP_site) +
    geom_sf(data = coast, color = "grey30", fill = "lightgrey",
            aes(size=0.1)) +
...
mpa <- plot_mpa(NCP_site , xlim=c(-180,180), ylim = c(-36, 31), legend_pos = "right")
ggsave(filename = here::here("outputs", "figures", "RLS_sites_with_protection.jpg"),
       plot = mpa, width=15, height = 7 )

mpa <-  plot_mpa(NCP_site , xlim=c(-180,180), ylim = c(-36, 31), legend_pos = "none")+
...
gold_coast_mpa<- plot_mpa( NCP_site,ylim = c(-32, -7),
                           xlim= c(110,160), legend_pos = "right")
caraib_mpa <- plot_mpa( NCP_site, ylim = c(-3, 18),
